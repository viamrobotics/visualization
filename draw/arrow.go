...
func (arrows Arrows) Draw(name string, options ...DrawableOption) *Drawing {
	...
	drawing := NewDrawing(config.UUID, config.Name, config.Parent, config.Pose, shape, NewMetadata(WithMetadataColors(arrows.Colors...), WithMetadataAxesHelper(config.ShowAxesHelper)))
	return drawing
}
