...
func (points Points) Draw(name string, options ...DrawableOption) *Drawing {
	...
	return NewDrawing(config.UUID, config.Name, config.Parent, config.Pose, shape, NewMetadata(WithMetadataColors(points.Colors...), WithMetadataAxesHelper(config.ShowAxesHelper)))
}
