...
func TestDrawnFrameSystem_Draw(t *testing.T) {
	...
	t.Run("ProducesTransformPerGeometry", func(t *testing.T) {
		...
		transforms, err := drawn.Draw("test")
		test.That(t, err, test.ShouldBeNil)
	...
	t.Run("EmptyName_FrameNamesUsedDirectlyAsPrefix", func(t *testing.T) {
		drawn := NewDrawnFrameSystem(fs, referenceframe.NewZeroInputs(fs))

		// With empty name, frame names are used as-is as the prefix for geometry labels
	...
	t.Run("WithParent_PropagatesParentToAllTransforms", func(t *testing.T) {
		...
		transforms, err := drawn.Draw("test", WithParent("robot-base"))
		test.That(t, err, test.ShouldBeNil)
