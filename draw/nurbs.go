...
func (nurbs Nurbs) Draw(name string, options ...DrawableOption) *Drawing {
	...
	return NewDrawing(config.UUID, config.Name, config.Parent, config.Pose, shape, NewMetadata(WithMetadataColors(nurbs.Colors...), WithMetadataAxesHelper(config.ShowAxesHelper)))
}
