...
func TestTransform(t *testing.T) {
	t.Run("NewTransform", func(t *testing.T) {
		...
		test.That(t, err, test.ShouldBeNil)
		transform, err := NewTransform("", "test", "world", spatialmath.NewPose(r3.Vector{X: 0, Y: 0, Z: 0}, &spatialmath.OrientationVectorDegrees{OX: 0, OY: 0, OZ: 1, Theta: 0}), geometry, metadataStruct)
		test.That(t, err, test.ShouldBeNil)
