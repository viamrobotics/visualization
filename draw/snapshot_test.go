...
func TestSnapshotBox(t *testing.T) {
	...
	// Per RDK's FrameSystemGeometries design: the geometry's pose defines its position
	// relative to the parent frame. The frame itself is at zero pose.
	addVoxelFrame := func(parent referenceframe.Frame, name string, x, y, z int) {
		...
		geomPose := spatialmath.NewPoseFromPoint(r3.Vector{
			X: float64(x) * voxelScale,
			Y: float64(y) * voxelScale,
			Z: float64(z) * voxelScale,
		})
...
func createBody(
	...
	err := snapshot.DrawFrame("", orbitFrameName, parent, spatialmath.NewPoseFromPoint(r3.Vector{X: x, Y: y, Z: 0}), nil, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSnapshotSphere(t *testing.T) {
	...
	err := snapshot.DrawFrame("", sunOrbitFrame, "world", spatialmath.NewPoseFromPoint(r3.Vector{X: 0, Y: 0, Z: 2000}), nil, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSnapshotArrows(t *testing.T) {
	...
	err = snapshot.DrawArrows(
		...
		spatialmath.NewZeroPose(),
		sphereArrowPoses,
		WithArrowsColors(NewColor(WithName("green"))),
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = snapshot.DrawArrows(
		...
		spatialmath.NewZeroPose(),
		capsuleArrowPoses,
		WithArrowsColors(NewColor(WithName("blue"))),
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSnapshotPoints(t *testing.T) {
	...
	err := snapshot.DrawPoints(
		...
		spatialmath.NewZeroPose(),
		gridPoints,
		WithPointsColors(NewColor(WithName("red"))),
	...
	err = snapshot.DrawPoints(
		...
		spatialmath.NewZeroPose(),
		spherePoints,
		WithPointsColors(sphereColors[0], sphereColors[1:]...),
		WithPointsSize(pointSize),
	)
	if err != nil {
		t.Fatal(err)
...
type sceneFixture struct {
	...
	FrameSystemInputs map[string][]float64 `json:"frame_system_inputs"`
	SceneConfig       sceneConfig          `json:"scene_config"`
	SceneCamera       SceneCamera          `json:"scene_camera,omitempty"`
}
...
func getColorForFrame(frameName string, config sceneConfig, palette map[string]Color) Color {
	if strings.HasPrefix(frameName, config.Arm) || strings.HasPrefix(frameName, "arm") {
		return palette["arm"]
	}
	if strings.HasPrefix(frameName, config.Camera) || strings.HasPrefix(frameName, "camera") {
		return palette["camera"]
	}
	if strings.HasPrefix(frameName, config.Vacuum) || strings.HasPrefix(frameName, "vacuum") {
		return palette["vacuum"]
	}
	if strings.HasPrefix(frameName, "sander") {
		return palette["sander"]
	}
	if strings.HasPrefix(frameName, "wall") {
		return palette["wall"]
	}
	if strings.HasPrefix(frameName, "floor") {
		return palette["floor"]
	}
	if strings.HasPrefix(frameName, "hardtop-middle-wall") {
		return palette["hardtop-middle-wall"]
	}
	return palette["default"]
}

func TestSnapshotScene(t *testing.T) {
	data, err := os.ReadFile(filepath.Join(".", fixturesDir, "scene_fixture.json"))
	if err != nil {
		t.Fatal(fmt.Errorf("failed to read fixture file: %w", err))
	}

	var fixture sceneFixture
	if err := json.Unmarshal(data, &fixture); err != nil {
		t.Fatal(fmt.Errorf("failed to unmarshal fixture: %w", err))
	}

	sceneCamera := NewSceneCamera(fixture.SceneCamera.Position, fixture.SceneCamera.LookAt)
	snapshot := NewSnapshot(WithSceneCamera(sceneCamera))

	var frameSystem referenceframe.FrameSystem
	if err := json.Unmarshal(fixture.FrameSystem, &frameSystem); err != nil {
		t.Fatal(fmt.Errorf("failed to unmarshal frame system: %w", err))
	}

	inputs := make(referenceframe.FrameSystemInputs)
	for k, v := range fixture.FrameSystemInputs {
		inputs[k] = referenceframe.FloatsToInputs(v)
	}

	colors := make(map[string]Color)
	colorPalette := map[string]Color{
		"default":             NewColor(WithName("red")),
		"arm":                 NewColor(WithName("orange")),
		"camera":              NewColor(WithName("blue")),
		"sander":              NewColor(WithName("cyan")),
		"vacuum":              NewColor(WithName("goldenrod")),
		"wall":                NewColor(WithName("gray")),
		"floor":               NewColor(WithName("slategray")),
		"hardtop-middle-wall": NewColor(WithName("magenta")),
	}

	for _, frame := range frameSystem.FrameNames() {
		color := getColorForFrame(frame, fixture.SceneConfig, colorPalette)
		colors[frame] = color
	}

	if err := snapshot.DrawFrameSystemGeometries(&frameSystem, inputs, colors); err != nil {
		t.Fatal(fmt.Errorf("failed to draw frame system geometries: %w", err))
	}

	if err := snapshot.Validate(); err != nil {
		t.Fatal(fmt.Errorf("snapshot validation failed: %w", err))
	}
