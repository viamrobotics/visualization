...
func TestGeometry(t *testing.T) {
	t.Run("DrawGeometries", func(t *testing.T) {
		...
		drawnGeometriesInFrame, err := NewDrawnGeometriesInFrame(geometriesInFrame, WithPerGeometriesColors(colors...))
		test.That(t, err, test.ShouldBeNil)
		test.That(t, drawnGeometriesInFrame, test.ShouldNotBeNil)
		test.That(t, len(drawnGeometriesInFrame.DrawnGeometries), test.ShouldEqual, 3)

		transforms, err := drawnGeometriesInFrame.Draw("")
		test.That(t, err, test.ShouldBeNil)
