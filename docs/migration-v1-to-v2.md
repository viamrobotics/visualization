...
Renamed. Color strings replaced by `[]draw.Color`. The `arrowHeadAtPose` parameter has been removed. New fields: `Name`, `Parent`, `ID`.
...
api.DrawGeometry(api.DrawGeometryOptions{
    ...
    Parent:   "world", // optional, defaults to "world"
})
```
...
The `shapes.Nurbs` struct is decomposed into individual fields. Color changed from a `string` name to `draw.Color`. New optional fields: `LineWidth`, `Parent`, `ID`.
...
Renamed. Wrapped in options struct. The function no longer divides by 1000 internally -- positions are passed through in millimeters as-is.
...
Resets the camera to the default position (looking at the origin from `{3000, 3000, 3000}`). No v1 equivalent.
...
draw.ColorFromColorRGBA(color.RGBA{R: 255, G: 0, B: 0, A: 255}) // from image/color.RGBA
```

For advanced use cases, the functional options constructor is available:

```go
color := draw.NewColor(draw.WithRGB(255, 0, 0))  // equivalent to draw.ColorFromRGB(255, 0, 0)
if z < 1 {
  color.SetAlpha(128)
}
```
...
// v2
chooser := draw.NewDefaultColorChooser()
color := chooser.Next() // returns draw.Color, cycles automatically
```
...
draw.DefaultLinePointColor // dark blue
