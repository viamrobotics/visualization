...
<Aside
	type="tip"
	title="A persistent measurement"
>
	`MeasureTool` is a throwaway two-point ruler: the line and its label disappear the moment you
	start a new measurement. For a labelled, multi-vertex annotation that stays in the scene, use the
	polyline gizmo on the [gizmos](/plugins/gizmos/) page instead. One difference to expect side by
	side: `MeasureTool` prints its distance in meters, while the gizmo's labels print millimeters to
