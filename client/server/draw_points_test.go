...
func TestDrawPoints(t *testing.T) {
	...
	t.Run("DrawPoints", func(t *testing.T) {
		...
		colors := make([]draw.Color, 0, nPath*nRing)
		...
		for i := range nPath {
			...
			for j := range nRing {
				...
				colors = append(colors, draw.NewColor(draw.WithRGB(
					uint8((cosT+1)*127.5),
					uint8((sinT+1)*127.5),
		...
		// Fill remaining with default red color
		defaultColor := draw.NewColor(draw.WithRGB(255, 0, 0))
		for len(colors) < len(points) {
