...
test.describe('deep-link selection', () => {
	...
	test('a comma-separated viz.select selects every named entity', async ({
		page,
		drawServer,
		drawScene,
	}) => {
		await page.goto(`?drawPort=${drawServer.port}&viz.select=lifecycle-box,lifecycle-line`)
		await expect(page.getByRole('heading', { name: 'World', exact: true })).toBeVisible({
			timeout: 15_000,
		})
		...
		await expect(
			page.getByLabel('Tree View').getByText('lifecycle-line', { exact: true })
		).toBeVisible({ timeout: 10_000 })

		const detailsPanels = page.getByRole('region', { name: 'Details panel' })
		await expect(detailsPanels).toHaveCount(2, { timeout: 10_000 })
		await expect(detailsPanels.filter({ hasText: 'lifecycle-box' })).toHaveCount(1)
		await expect(detailsPanels.filter({ hasText: 'lifecycle-line' })).toHaveCount(1)
	})
})
