...
describe('isPerVertexColors', () => {
	...
	it('returns true when colors length matches numPoints * 4 (RGBA)', () => {
		expect(isVertexColors(new Uint8Array(4), 1)).toBe(true) // 1 point, RGBA
		expect(isVertexColors(new Uint8Array(40000), 10000)).toBe(true) // 10k points, RGBA
	})
	...
	it('returns false for a single uniform color with multiple points', () => {
		...
		expect(isVertexColors(new Uint8Array(4), 2)).toBe(false) // 1 RGBA color, 2 points
	})
