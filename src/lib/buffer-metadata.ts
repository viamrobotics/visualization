...
export const parseBuffer = (data: Uint8Array, metadata?: BufferMetadata): TypedArray => {
	if (!metadata || metadata.type.length === 0) {
		// Fallback: assume Float32Array
		return new Float32Array(data.buffer, data.byteOffset, data.byteLength / 4)
	}

	// Check if all fields are the same type
	...
	// Fast path: if all fields are the same type and size, we can create a single array
	if (allSameType && allSameSize) {
		return createTypedArray(data, firstType, firstSize)
	}

	// Complex case: mixed types - convert everything to Float32 for Three.js
	// This handles interleaved buffers with different field types
...
/**
 * Create the appropriate typed array based on type and size
 */
const createTypedArray = (data: Uint8Array, type: BufferDataType, size: number): TypedArray => {
	...
	switch (type) {
		case BufferDataType.INT:
			...
			if (size === 2) return new Int16Array(buffer, byteOffset, byteLength / 2)
			if (size === 4) return new Int32Array(buffer, byteOffset, byteLength / 4)
			break
		case BufferDataType.UINT:
			if (size === 1) return new Uint8Array(buffer, byteOffset, byteLength)
			if (size === 2) return new Uint16Array(buffer, byteOffset, byteLength / 2)
			if (size === 4) return new Uint32Array(buffer, byteOffset, byteLength / 4)
			break
		case BufferDataType.FLOAT:
			if (size === 4) return new Float32Array(buffer, byteOffset, byteLength / 4)
			if (size === 8) return new Float64Array(buffer, byteOffset, byteLength / 8)
			break
	}
...
/**
 * Read a single value from a buffer at a specific offset
 */
const readValue = <T extends TypedArray>(
	...
	switch (type) {
		case BufferDataType.INT:
			...
			if (size === 2) return view.getInt16(0, true) // little-endian
