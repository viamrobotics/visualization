...
import { traits, spawnTransformEntity, spawnDrawingEntities } from '$lib/ecs'
...
export const spawnSnapshotEntities = (world: World, snapshot: Snapshot): Entity[] => {
	...
	for (const transform of snapshot.transforms) {
		entities.push(spawnTransformEntity(world, transform, traits.SnapshotAPI))
	}

	for (const drawing of snapshot.drawings) {
		entities.push(...spawnDrawingEntities(world, drawing, traits.SnapshotAPI))
	}
