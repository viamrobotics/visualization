...
Allocates a batched instance in `ShapeBatches` for every entity with `Box` +
`WorldMatrix` traits, instead of drawing a mesh per box. Trait events are
...
<script lang="ts">
	...
	const addInstance = (entity: Entity) => {
		const ids = shapes.add(entity, 'box')
	 ...
	 * filter to box entities before touching the dirty sets. `instanceIdsByEntity`
	 * catches entities whose `Box` trait was just removed.
	 */
	const enqueue = (dirty: Set<Entity>) => (entity: Entity) => {
		if (!entity.has(traits.Box) && !instanceIdsByEntity.has(entity)) return
		dirty.add(entity)
		schedule()
	}
